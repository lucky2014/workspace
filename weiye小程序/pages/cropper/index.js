...
var initDragCutH = 0
var initDragCutT = 0
var wHeight = wx.getSystemInfoSync().windowHeight*(750/wx.getSystemInfoSync().windowWidth)-100
var wWidth = wx.getSystemInfoSync().windowWidth*(750/wx.getSystemInfoSync().windowWidth)
...
Page({
  ...
  onReady: function () {
    ...
    wx.getImageInfo({
      ...
      success: function success(res) {
        ...
        var windowRadio = wWidth/wHeight;
        // 根据图片的宽高显示不同的效果   保证图片可以正常显示
        if (innerAspectRadio >= windowRadio) {
          _this.setData({
            cropperW: windowWRPX,
            cropperH: windowWRPX / innerAspectRadio,
            // 初始化left right
            cropperL: Math.ceil((windowWRPX - windowWRPX) / 2),
            cropperT: Math.ceil((windowWRPX - windowWRPX / innerAspectRadio) / 2<0?0:(windowWRPX - windowWRPX / innerAspectRadio) / 2),
            // 裁剪框  宽高  
            cutW: windowWRPX - 200,
          ...
          _this.setData({
            cropperW: wHeight * innerAspectRadio,
            cropperH: wHeight,
            // 初始化left right
            cropperL: Math.ceil((windowWRPX - wHeight * innerAspectRadio) / 2),
            cropperT: Math.ceil((windowWRPX - windowWRPX) / 2),
            // 裁剪框的宽高
            cutW: wHeight * innerAspectRadio - 50,
            cutH: wHeight-50,
            cutL: Math.ceil((wHeight * innerAspectRadio - (wHeight * innerAspectRadio - 50)) / 2),
            cutT: Math.ceil(50 / 2),
            // 图片缩放值
            scaleP: res.width * pixelRatio / windowWRPX,
            // 图片原始宽度 rpx
  ...
  getImageInfo () {
    ...
    if(_this.data.imageSrc.indexOf("diantimes")!=-1){
      wx.downloadFile({
        ...
        success: function (res) {
          ...
          wx.canvasToTempFilePath({
            ...
            success: function (res) {
              ...
              arr[_this.data.index].img = res.tempFilePath;
              prevPage.setData({
                contect: arr
              })
              wx.navigateBack();
